        ...
        binding.button.setOnClickListener {
            if (isNetworkAvailable()) {
                ...
                if (email.isNotEmpty() && pass.isNotEmpty() && confirmPass.isNotEmpty()) {
                    if (pass == confirmPass) {
                        if (pass.length > 7) { // Check if password is longer than 7 characters
                                ...
                                .addOnCompleteListener { task ->
                                    if (task.isSuccessful) {
                                        ...
                                        if (task.exception is FirebaseAuthWeakPasswordException) {
                                            ...
                                            showErrorDialog("Password is too weak. Please choose a stronger password.")
                                        } else if (task.exception is FirebaseAuthInvalidCredentialsException) {
                                            // Show error dialog for invalid email
                                            showErrorDialog("Invalid email format.")
                                        } else if (task.exception is FirebaseAuthUserCollisionException) {
                                            // Show error dialog for existing user
                                            showErrorDialog("Account already exists with this email.")
                                        } else {
                                            // Show a generic error dialog
                                            showErrorDialog(task.exception?.message ?: "Registration failed")
                                        }
                                    }
                                }
                        } else {
